...
router.post('/register', async (req, res) => {
	...
	try {
		...
		return res
			.status(400)
			.send({ error: 'Oops... there was an error with your registration!' })
	}
})
...
router.post('/authenticate', async (req, res) => {
	...
	try {
		...
		if (!(await bcrypt.compare(password, user.password)))
			return res
				.status(401)
				.send({ error: 'The username or pasword provided is invalid!' })
...
router.post('/reset-password', async (req, res) => {
	...
	try {
		...
		if (token !== user.passwordResetToken)
			return res
				.status(400)
				.send({ error: 'The password reset token you provided is invalid!' })
